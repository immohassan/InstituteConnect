                ...
                @if(auth()->user()->societies->count() > 0)
                    <div class="list-group">
                        @foreach(auth()->user()->societies as $society)
                            <a href="{{ route('posts.index', ['society_id' => $society->id]) }}" class="list-group-item list-group-item-action d-flex align-items-center">
                                @if($society->logo)
                                    <img src="{{ Storage::url($society->logo) }}" alt="{{ $society->name }}" class="avatar avatar-sm me-3">
                                @else
            ...
            <div class="card-body">
                ...
                @if($announcements->count() > 0)
                    @foreach($announcements as $announcement)
                        @include('components.announcement-card', ['announcement' => $announcement, 'compact' => true])
                    @endforeach
                    
                    <div class="text-center mt-3">
                        <a href="{{ route('announcements.index') }}" class="btn btn-sm btn-outline-primary">View All Announcements</a>
                    </div>
                @else
