...
<?php
    ...
    $f_posts = $posts->filter(function ($post) use ($followingIds) {
        return $followingIds->contains($post->user_id);
    });
?>
...
<?php $__env->startSection('content'); ?>
    ...
    <div class="container main-section" id="post-container">
        <div class="tab-container mb-5 position-relative">
            <div class="tab active" data-target="#explore">Explore</div>
            <div class="tab" data-target="#followings">Followings</div>
        ...
        <div class="tab-content">
            ...
            <div id="followings" class="tab-pane">
                <?php if(count($f_posts) > 0): ?>
                    <?php $__currentLoopData = $f_posts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $post): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="card mb-4">
                            <div class="card-header">
                                <div class="d-flex align-items-center profile-header"
                                    data-url="<?php echo e(route('profile.show', ['id' => $post->user->id])); ?>">
    ...
    <script>
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    `;
                        ...
                        $('#notification-bell .bg-danger')
                            .show(); // Show the red dot if there are new notifications
        ...
        $(document).ready(function() {
            ...
            function moveUnderline($activeTab) {
                const tabContainerOffset = $('.tab-container').offset().left;
                const tabOffset = $activeTab.offset().left;
                const underlineWidth = $activeTab.outerWidth();
                const left = tabOffset - tabContainerOffset;
            ...
            $('.comment-form').on('submit', function(e) {
                ...
                `;
                        ...
                        $.ajax({
                            ...
                            data: {
                                contents: "Hey " + res.ReceptorUserName + "! " + res
                                    .InitiatorName + " commented on your post!",
            ...
            $('#attachmentt').on('change', function(e) {
                        ...
                        if (res.liked) {
                            ...
                            if (res.postUserId && res.postUserSubscriptionId) {
                                ...
                                $.ajax({
                                    ...
                                    data: {
                                        contents: "Hey " + res.ReceptorUserName + "! " +
                                            res.InitiatorName + " liked your post!",
                                        subscriptionIds: res.postUserSubscriptionId,
                                        url: window.location.href, // or the post URL
                                        userId: res.ReceptorUserId,
                                        initiatorId: res.InitiatorId
